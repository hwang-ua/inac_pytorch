...
from core.network import network_utils, network_architectures
...
class MLPCont(nn.Module):
    ...
    """
    def __init__(self, device, obs_dim, act_dim, hidden_sizes, action_range=1.0):
        super().__init__()
        self.device = device
        body = network_architectures.FCBody(device, obs_dim, hidden_units=tuple(hidden_sizes))
...
class MLPDiscrete(nn.Module):
    def __init__(self, device, obs_dim, act_dim, hidden_sizes):
        super().__init__()
        self.device = device
        body = network_architectures.FCBody(device, obs_dim, hidden_units=tuple(hidden_sizes))
        body_out = obs_dim if hidden_sizes==[] else hidden_sizes[-1]
...
class PolicyFactory:
    ...
    def get_policy_fn(cls, cfg):
        if cfg.policy_fn_config['policy_type'] == "policy-cont":
            return lambda: MLPCont(cfg.device, np.prod(cfg.policy_fn_config['in_dim']),
            ...
            return lambda: MLPDiscrete(cfg.device, np.prod(cfg.policy_fn_config['in_dim']),
                                           cfg.action_dim, cfg.policy_fn_config['hidden_units']
                                           )
        else:
